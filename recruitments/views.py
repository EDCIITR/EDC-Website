...
def index(request):
	...
	try:
		...
		return render_to_response('recruitments/index.html',{'name':'Recruitments','list':menu,'form':form,'setup':s,'coordinators':coordinators},context_instance=RequestContext(request))
	
	except ObjectDoesNotExist:
		future=None
		past = None
		try:
			future = Setup.objects.filter(date_recruitment_starts__gt=datetime.now()).order_by('date_recruitment_starts')
			future = future[0]
		except IndexError:
			pass
		
		try:
			past = Setup.objects.filter(date_recruitment_ends__lt=datetime.now()).order_by('-date_recruitment_ends')	
			past = past[0]
		except IndexError:
			pass

		return render_to_response('recruitments/none.html',{'name':'Recruitments','list':menu,'future':future,'past':past},context_instance=RequestContext(request))
